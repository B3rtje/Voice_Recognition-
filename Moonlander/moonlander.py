...
class ReplayBuffer(): 
    ...
    # New function to store transaction in the memory
    def store_transition(self, state, action, reward, state_, done): 
        index = self.mem_cntr % self.mem_size # Is to know the position! 
        ...
        self.mem_cntr += 1   #important because otherwise we write our memory over and over!! 

    def sample_buffer(self, batch_size):
        max_mem = min(self.mem_cntr, self.mem_size)
        batch = np.random.choice(max_mem, batch_size, replace=False) # Replace = False you can't sample a sample that already is)
...
#This is the harder part of the deep Q network! 
class Agent(): 
    def __init__(self, lr, gamma, n_actions, epsilon, batch_size, input_dims, epsilon_dec=1e-3, eps_end=0.01, 
        ...
        self.learn_step_counter = 0 # How many time you cal the learn function 
        ...
        # Onderstaande q_next --> just a formality won't optimize network
        self.q_next.compile(optimizer=Adam(learning_rate=lr), loss='mean_squared_error')

#Function for interface between agent & his memory! 

    def store_transition(self, state, action, reward, new_state, done): 
        self.memory.store_transition(state, action, reward, new_state, done)

#Function for taking an action based on the observation of the environment! 
    ...
    def choose_action(self, observation): 
        if np.random.random() < self.epsilon: 
            ...
            action = tf.math.argmax(actions, axis=1).numpy()[0] #[0] is because it returns an tupple!
